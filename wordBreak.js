...
function wordBreak(textPageFontName, targetClasssName, callBack){
			...
			if(pageWords.length === 1){
				...
				for(var i = 0; i < pageWords.length; i++){
					...
					// 최대길이 이상일 때 
						...
						//하지만 해당 2단어만으로 라인이 오버할 경우, i-1값을 본문 텍스트에 넣고 라인 컨테이너에서는 i번째만 넣는다. 
						// debugger;
						lineContainer = pageWords[i-1] + pageWords[i];
						tempP.text(lineContainer);
						if(tempP.width() > pageContentTextWidth){
							lineContainer = pageWords[i];
							resultPageText += pageWords[i-1] + '<br>';
							tempP.text("");
						} else {
							tempP.text("");	
						}
